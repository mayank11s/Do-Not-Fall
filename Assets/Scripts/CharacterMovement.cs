...
public class CharacterMovement : MonoBehaviour
{
    ...
    private bool HasPlatformAhead()
    {
        Vector3 checkPosition =
            transform.position +
            moveInput.normalized * platformCheckDistance +
            Vector3.up * platformCheckHeight;

        return Physics.CheckBox(
            checkPosition,
            platformCheckHalfExtents,
            Quaternion.identity,
            platformLayer,
    ...
    private void UpdateAnimation(bool grounded)
    {
        ...
        // Safety in case the character falls from a platform
        // without using our jump.
        if (!grounded && !isJumping)
        {
            animator.SetBool(InAirHash, true);
        }
    ...
    private void OnDrawGizmosSelected()
    {
        ...
        Vector3 checkPosition =
            transform.position +
            direction * platformCheckDistance +
            Vector3.up * platformCheckHeight;

        Gizmos.DrawWireCube(
            checkPosition,
            platformCheckHalfExtents * 2f
        );
    }
